...
/// Get user by id.
pub async fn read(user_id: String, conn: &DatabaseConnection) -> Result<User> {
...
pub async fn get_user_by_discord_id(discord_id: String, conn: &DatabaseConnection) -> Result<Option<User>> {
    let response: Option<user::Model> = user::Entity::find()
        .filter(user::Column::DiscordId.contains(&discord_id.clone()))
        .one(conn)
        .await?;
