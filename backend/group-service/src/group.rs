...
pub async fn join_group(database: web::Data<Database>, decoding_key: web::Data<DecodingKey>, req: HttpRequest) -> impl Responder {
    let group_id = req.match_info().get("id").unwrap().to_string();
    let user_response = authorize(&req, decoding_key.get_ref()).await;
...
pub async fn leave_group(database: web::Data<Database>, decoding_key: web::Data<DecodingKey>, req: HttpRequest) -> impl Responder {
    let group_id = req.match_info().get("id").unwrap().to_string();
    let user_response = authorize(&req, decoding_key.get_ref()).await;
